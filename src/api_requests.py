...
# add by gq [2026-04-28：复用OpenAI兼容协议接入Kimi和Minimax]
class BaseOpenAICompatibleProcessor(BaseOpenaiProcessor):
    def __init__(self, api_key_env: str, base_url_env: str, default_base_url: str, default_model_env: str, default_model: str):
        load_dotenv()  # add by gq [2026-04-28：确保先加载.env再读取兼容模型配置]
    ...
    def send_message(
        ...
        **kwargs
        ...
        if is_structured and response_format is not None:
            schema_text = json.dumps(response_format.model_json_schema(), ensure_ascii=False)
            messages[0]["content"] = (
                f"{system_content}\n\n"
        ...
        if is_structured and response_format is not None:
            try:
                ...
                return response_format.model_validate(parsed_dict).model_dump()
            except Exception as err:
                print(f"Error processing structured compatible response: {err}")
                ...
                try:
                    ...
                    print(f"Compatible response reparse failed: {reparse_err}")
                    return content
    ...
    def __init__(self, provider: Literal["openai", "ibm", "gemini", "dashscope", "kimi", "minimax", "agicto"] ="dashscope"):
        ...
        if self.provider == "openai":
            ...
            self.processor = BaseOpenAICompatibleProcessor(
                ...
                default_model_env="MOONSHOT_MODEL",
    ...
    def get_answer_from_rag_context(self, question, rag_context, schema, model):
        ...
        if 'step_by_step_analysis' not in answer_dict:
            answer_dict = {
                ...
                "reasoning_summary": "",
                "relevant_pages": [],
                "final_answer": answer_dict.get("final_answer", "N/A") if isinstance(answer_dict, dict) else str(answer_dict)
            }
        return answer_dict
