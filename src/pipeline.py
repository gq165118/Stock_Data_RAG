...
class RunConfig:
    ...
    # modified by gq [2026-04-28：默认问答模型切换为Kimi，便于使用OpenAI兼容接口运行]
    api_provider: str = "kimi" # openai/dashscope/kimi/minimax
    answering_model: str = "moonshot-v1-8k" # kimi: moonshot-v1-8k；minimax: MiniMax-Text-01
    # mod end
    config_suffix: str = ""
...
max_nst_o3m_config = RunConfig(
    ...
    api_provider="dashscope",  # add by gq [2026-04-28：保留原Qwen配置，避免默认Kimi影响历史配置]
...
# add by gq [2026-04-28：增加Kimi问答配置，使用Moonshot OpenAI兼容接口]
kimi_config = RunConfig(
    use_serialized_tables=False,
    parent_document_retrieval=True,
    llm_reranking=True,
    parallel_requests=2,
    submission_name="Local RAG with Kimi",
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = Kimi moonshot-v1-8k",
    api_provider="kimi",
    answering_model="moonshot-v1-8k",
    config_suffix="_kimi"
)
# add end

# add by gq [2026-04-28：增加Minimax问答配置，使用Minimax OpenAI兼容接口]
minimax_config = RunConfig(
    use_serialized_tables=False,
    parent_document_retrieval=True,
    llm_reranking=True,
    parallel_requests=2,
    submission_name="Local RAG with Minimax",
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = MiniMax-Text-01",
    api_provider="minimax",
    answering_model="MiniMax-Text-01",
