...
class VectorRetriever:
    ...
    # add by gq [2026-04-28：绕开FAISS在Windows中文路径下直接读文件失败的问题]
    @staticmethod
    def _read_faiss_index(index_path: Path):
    ...
    def _load_dbs(self):
        ...
        for document_path in all_documents_paths:
            ...
            try:
                # modified by gq [2026-04-28：使用Python读取二进制内容以兼容中文项目路径]
                vector_db = self._read_faiss_index(vector_db_files[stem])
                # mod end
            except Exception as e:
